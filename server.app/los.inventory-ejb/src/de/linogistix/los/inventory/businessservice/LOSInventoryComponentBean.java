			...
			for (LOSUnitLoad ul : uls) {
				...
				for (StockUnit su : ul.getStockUnitList()) {
					...
					if ((ul.getPackageType().equals(LOSUnitLoadPackageType.OF_SAME_LOT) || ul.getPackageType().equals(LOSUnitLoadPackageType.OF_SAME_LOT_CONSOLIDATE)) && su.getLot() != null
						...
						log.error("found UnitLoad with StockUnits of different lot: " + ul.toShortString());
					...
					if (su.getReservedAmount().compareTo(new BigDecimal(0)) > 0) {
						List<LOSPickingPosition> pickList = pickPosService.getByPickFromStockUnit(su);
						if (pickList.size() < 1) {
							log.error("found StockUnit with reservation but no pick request position " + su.toShortString());
						}
					}
				}
			}
		...
		try {
			...
			for (BODTO<LOSAdvice> ad : adviceQuery.queryAllHandles(new QueryDetail(0, Integer.MAX_VALUE))) {
				...
				if (newA.compareTo(old) != 0) {
					...
					log.info("NO CHANGE of amount for " + a.toShortString());
				}
			}
		...
		try {
			...
			for (LOSGoodsReceiptPosition pos : list) {
				...
				if (pos.getStockUnitStr() == null) {
					...
					log.error("GOING TO CORRECT StockUnitStr - expected " + su.toShortString() + " but was null ");
					pos.setStockUnit(su);
				}
				...
				if (pos.getAmount().compareTo(new BigDecimal(0)) == 0) {
					...
					List<LOSStockUnitRecord> suRecords = recordService.getByStockUnitAndType(su, LOSStockUnitRecordType.STOCK_CREATED);
					if (suRecords.size() != 1) {
						log.error("NOT ONE CREATION RECORD FOUND - skip: " + pos.toShortString());
						continue;
					}

					pos.setAmount(suRecords.get(0).getAmount());
					log.info("CORRECTED null amount: " + pos.toShortString());
				}
			}
		} catch (Throwable t) {
			log.error(t.getMessage(), t);
