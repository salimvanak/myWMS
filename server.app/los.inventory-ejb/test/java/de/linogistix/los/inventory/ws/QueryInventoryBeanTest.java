...
public class QueryInventoryBeanTest extends TestCase {
	...
	public void testGetInventoryList(){
		...
		try {
			...
			ret = proxy.getInventoryByArticle("Test Client", InventoryTestTopologyRemote.ITEM_A1_NUMBER,true);
			...
			InventoryTO to = proxy.getInventoryByLot("Test Client", InventoryTestTopologyRemote.ITEM_A1_NUMBER, InventoryTestTopologyRemote.LOT_N1_A1_NAME);
			logger.info(to.toString());
			
			try{
				to = proxy.getInventoryByLot("Test Client", InventoryTestTopologyRemote.ITEM_A2_NUMBER, InventoryTestTopologyRemote.LOT_N1_A1_NAME);
				fail("LOT MISMATCH");
			} catch (InventoryException_Exception ex){
				// OK
			}
