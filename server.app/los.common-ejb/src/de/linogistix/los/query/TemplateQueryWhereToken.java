...
public class TemplateQueryWhereToken implements Serializable {
	...
	public TemplateQueryWhereToken(String operator, String parameter, String parameterName, Object value) {
		setValue(value);
		setOperator(operator);
		setParameter(parameter);
	...
	public String toParameterName(String parameter) {
		...
		if (getValue() == null) {
			...
			// we want to make sure that two parameters with different values on the 
			// same column have different names.
			return ret + Integer.toHexString(getValue().hashCode()%10000);
		}
	}
