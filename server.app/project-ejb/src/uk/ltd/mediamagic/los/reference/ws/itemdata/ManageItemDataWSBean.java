 ...
 *  Project myWMS-LOS
 */
package uk.ltd.mediamagic.los.reference.ws.itemdata;
...
@WebService(endpointInterface = "uk.ltd.mediamagic.los.reference.ws.itemdata.ManageItemDataWS")
...
public class ManageItemDataWSBean implements ManageItemDataWS {
	...
	public void updateItemData(UpdateItemDataRequest updateReq) throws ManageItemDataWSFault{
		...
		if(item == null){
			...
			item.setWeight(updateReq.getWeight());
			item.setHeight(updateReq.getHeight());
			item.setWidth(updateReq.getWidth());
			item.setDepth(updateReq.getDepth());
		}
		// otherwise update the existing
		else{
			try {
				if (!Objects.equals(item.getHandlingUnit().getUnitName(), reqUnit.getUnitName())) {
			...
			try {
				if (!Objects.equals(item.getScale(), updateReq.getScale())) {
			...
			try {
				if (item.isLotMandatory() != updateReq.isLotMandatory()) {
			...
			try {
				if (item.getSerialNoRecordType() != updateReq.getSerialNoRecordType()) {
